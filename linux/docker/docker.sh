...
if [ "$(id -u)" -ne 0 ]; then
    if ! command -v sudo >/dev/null 2>&1; then
        log_error "需要 root 权限运行，且系统未安装 sudo，请改用 root 用户。"
    fi
    # bash <(curl ...) 运行时 $0 是 /dev/fd/*，sudo 会关闭该 fd，先落盘再提权
    SELF="$(mktemp "${TMPDIR:-/tmp}/xcript-docker-XXXXXX.sh" 2>/dev/null || echo "/tmp/xcript-docker-$$.sh")"
    if ! cat "$0" > "$SELF" 2>/dev/null; then
        log_error "读取自身脚本失败，请先下载到本地再运行。"
    fi
    chmod +x "$SELF"
    log_warn "需要 root 权限，正在通过 sudo 重新执行..."
    exec sudo bash "$SELF" "$@"
fi
...
if [ "$USE_MIRROR" = true ]; then
    ...
    if [ -f /etc/docker/daemon.json ]; then
        DAEMON_BAK="/etc/docker/daemon.json.bak.$(date +%Y%m%d%H%M%S)"
        cp -a /etc/docker/daemon.json "$DAEMON_BAK"
        log_warn "已备份原有 daemon.json -> ${DAEMON_BAK}"
    fi
...
if [[ "$DOCKER_VER" != "未安装成功" ]]; then
    echo -e "${GREEN}[OK] Docker 环境安装与配置全部完成！${NC}"
    ...
    echo -e "${RED}[ERR] 安装可能存在异常，请检查上方日志。${NC}"
fi
echo "-------------------------------------------------------------------"
