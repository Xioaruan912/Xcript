...
更新分三层，启动时**不会**因为联网而卡住：

- **启动器自我更新** `codex.bat`：启动时比对自身版本与远端 `bat=`，落后就下载新版，用一个临时 helper 在本进程退出后**替换掉你正在运行的那个 bat**（桌面 / 任意目录都行），再用新版重启。这份 bat 从此会跟着更新。
- 启动器**缓存检查**：缓存命中时做一次极快版本比对（只取 `version.txt`，6 秒超时，优先走本地代理，失败静默跳过），缓存的核心脚本落后就自动刷新。
- 核心脚本 `codex-switcher.ps1`：**交互启动时只读本地版本缓存，绝不联网**，保证秒开。有新版时只提示「运行 -Update 更新」，不会自动下载。

`-noproxy` 会跳过所有联网更新检查，只跑本地缓存。

| 命令 | 行为 |
| --- | --- |
| `codex.bat` | 自我更新（如有新版）+ 缓存可用即秒开；缓存脚本落后则自动刷新 |
| `codex.bat -force` | 忽略缓存，重新下载核心脚本 |
| `codex.bat -noproxy` | 跳过网络，纯离线使用本地缓存 |
| `codex-switcher.ps1 -Update` | 强制探测并下载核心脚本 / 启动器 |
| `codex-switcher.ps1 -Doctor` | 查看本地 / 远端版本与更新状态 |

### 启动器是怎么替换自己的

Windows 上正在运行的 `.bat` 没有文件锁，可以被覆盖；但如果边运行边覆盖，cmd 继续逐行读取会读到新内容导致执行错乱。所以做法是：

1. 下载新版到 `%TEMP%`，校验内容。
2. 生成一个临时 helper，内容是「等本进程退出 -> 复制新版覆盖原 bat -> 用新版重启 -> helper 自删」。
3. 当前 bat 立刻退出，helper 完成替换后拉起新版。

替换的是「你实际运行的那个 bat」的绝对路径，所以放在桌面、开始菜单或任意目录都有效。
